...
- Phaser now starts from server setup, sends active-owner movement/fire intents to the room, and applies live room snapshots back into visible vehicle position, HP, alive state, movement units, facing, aim, KO/Void Drop reason, and compact authoritative crater events on every browser. Projectile launch, flight, collision, terrain deformation, shared damage rules, Nova knockback, settlement, HP KO, and Void Dropped outcomes are server-owned. Round and match lifecycle remain the next server-authority slices.
...
For online v1, the next saved path is to complete server-owned turn/round flow, then Best-of-1/Best-of-3 scoring and same-room match reset.
