...
- `npm run start:public` now serves the built client plus Colyseus from one localhost-bound port for tunnel-based internet playtests. Use `npm run build` first, then expose `http://127.0.0.1:2567` through a trusted tunnel and share the tunnel URL with `?onlinePanel=1`. Default and public-preview startup stay bound to `127.0.0.1`; direct all-interface binding requires explicitly setting `HOST=0.0.0.0`.
